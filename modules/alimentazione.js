...
const catOf   = (d, id) => ingCats(d).find(c => c.id === id) || { name: 'Altro', emoji: '📦' };
...
const fpKey = (off, day, mid) => wKey(off) + '-' + day + '-' + mid;
function monOf(off) {
...
function rLog() {
  ...
  const data = D(), ts = today();
  const logDay = logs(data).find(l => l.date === ts) || { date: ts, meals: [] };
  const tot = logDay.meals.reduce((a, m) => {
    const t = totals(m.ingredients);
    return { kcal: a.kcal + t.kcal, protein: +(a.protein + t.protein).toFixed(1), carbs: +(a.carbs + t.carbs).toFixed(1), fat: +(a.fat + t.fat).toFixed(1), fiber: +(a.fiber + t.fiber).toFixed(1) };
  }, { kcal: 0, protein: 0, carbs: 0, fat: 0, fiber: 0 });
  const bmr = data.health?.bmr || 0, def = data.health?.caloricDeficit || 0, goal = bmr ? bmr - def : null;
  const pct = goal ? Math.min(100, Math.round(tot.kcal / goal * 100)) : 0;
  const diff = goal ? goal - tot.kcal : 0;
  ...
  let h = '<div class="card-dark mb-12">' +
    ...
    '<circle cx="48" cy="48" r="' + R + '" fill="none" stroke="var(--black3)" stroke-width="8"/>' +
    '<circle cx="48" cy="48" r="' + R + '" fill="none" stroke="var(--lime)" stroke-width="8" stroke-linecap="round" stroke-dasharray="' + C + '" stroke-dashoffset="' + (C - C * pct / 100) + '" transform="rotate(-90 48 48)"/>' +
    '</svg>' +
    '<div style="position:absolute;font-size:17px;font-weight:900;color:var(--white)">' + pct + '%</div></div>' +
    '<div style="flex:1">' +
    '<div style="font-size:38px;font-weight:900;color:var(--white);line-height:1">' + fmtNum(tot.kcal) + '</div>' +
  ...
  MEAL_TYPES.forEach(type => {
    ...
    h += '<div class="card-dark mb-12">' +
      '<div class="flex-between mb-8"><div>' +
      ...
      '<button onclick="openAddMealM(\'' + type + '\')" style="flex:1;padding:8px 4px;border-radius:var(--radius-md);border:none;font-size:10px;font-weight:700;cursor:pointer;background:var(--black3);color:var(--gray2)">✏️ Manuale</button>' +
      '<button onclick="openLibPicker(\'' + type + '\')" style="flex:1;padding:8px 4px;border-radius:var(--radius-md);border:none;font-size:10px;font-weight:700;cursor:pointer;background:var(--black3);color:var(--gray2)">📚 Libreria</button>' +
      '<button onclick="logFromPiano(\'' + type + '\')" style="flex:1;padding:8px 4px;border-radius:var(--radius-md);border:none;font-size:10px;font-weight:700;cursor:pointer;background:var(--black3);color:var(--gray2)">📅 Piano</button>' +
      '</div>';
    ...
    if (tm.length) {
      h += '<div style="border-top:1px solid var(--black3);padding-top:8px">' +
        tm.map(m => {
          const t = totals(m.ingredients);
          const idx = logDay.meals.indexOf(m);
          return '<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--black3)">' +
            ...
            '<button onclick="removeLog(\'' + ts + '\',' + idx + ')" style="background:none;border:none;color:#FF6B6B;font-size:13px;cursor:pointer">✕</button>' +
            '</div></div>';
        }).join('') + '</div>';
    } else {
...
window.openLibPicker = function(type) {
  const data = D();
  const ms = meals(data).filter(m => m.type === type);
  ...
  if (!ms.length) {
    ...
    list.innerHTML = ms.map(m => {
      ...
      return '<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--black3)">' +
        ...
        '<button onclick="pickLibMeal(\'' + type + '\',\'' + mId + '\')" class="btn btn-lime btn-sm">+ Log</button>' +
        '</div>';
    }).join('');
  }
...
window.pickLibMeal = function(type, mId) {
  const m = meals(D()).find(m => m.id === mId);
  if (!m) return;
  addToLog({ ...m, type });
  closeModal('modal-lib-picker');
  rLog();
};
...
window.logFromPiano = function(type) {
  ...
  const slot = (plan(data)[key] || []).filter(e => e.type === type);
  if (!slot.length) { alert('Nessun pasto di tipo ' + type + ' nel piano di oggi.'); return; }
  slot.forEach(e => {
    const m = meals(data).find(m => m.id === e.mealId);
...
function rPasti() {
  ...
  let h = '<div class="flex-between mb-12"><div style="font-size:16px;font-weight:800;color:var(--black)">Pasti salvati</div>' +
    '<button onclick="openNewMealM()" class="btn btn-dark btn-sm">+ Nuovo</button></div>';
  if (!ms.length) {
    ...
    MEAL_TYPES.forEach(type => {
      const tm = ms.filter(m => m.type === type);
      if (!tm.length) return;
      ...
      tm.forEach(m => {
        ...
        h += '<div class="card-dark" style="margin-bottom:8px">' +
          ...
          '<div style="font-size:15px;font-weight:700;color:var(--white)">' + m.name + '</div>' +
          ...
          '<button onclick="delMeal(\'' + m.id + '\')" style="background:none;border:none;color:#FF6B6B;font-size:13px;cursor:pointer">🗑️</button>' +
          '</div></div>' +
          '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:6px">' +
          '<span style="font-size:13px;font-weight:700;color:var(--lime)">' + t.kcal + ' kcal</span>' +
...
function rIngredienti() {
  ...
  if (!all.length) {
    ...
    Object.entries(grouped).forEach(function([catId, items]) {
      const c = catOf(data, catId);
      const isCol = collapsed.has(catId);
...
function rPiano() {
  ...
  if (pianoSub === 'personale') rPianoP();
  else rPianoF();
}
window.pianoTab = s => { pianoSub = s; rPiano(); };
...
function navHtml() {
  ...
  return '<div class="flex-between mb-14">' +
    ...
    DAYS_SHORT.map((d, i) =>
      '<button onclick="setPD(' + i + ')" style="flex-shrink:0;padding:6px 10px;border-radius:99px;border:none;font-size:11px;font-weight:700;cursor:pointer;background:' + (pianoDay === i ? 'var(--lime)' : 'var(--black3)') + ';color:' + (pianoDay === i ? 'var(--black)' : 'var(--gray2)') + '">' + d + '</button>'
    ).join('') +
    '</div>' +
    '<div style="font-size:16px;font-weight:800;color:var(--black);margin-bottom:12px">' + DAYS_IT[pianoDay] + '</div>';
}
...
function rPianoP() {
  ...
  let h = navHtml();
  const key = pKey(pianoWeek, pianoDay);
  MEAL_TYPES.filter(t => t !== 'Snack').forEach(type => {
    ...
    h += '<div class="card mb-8" style="padding:14px"><div class="flex-between mb-8"><div>' +
      ...
      (slotMs.length ? slotMs.map(m =>
        ...
        '<div style="display:flex;align-items:center;gap:8px">' +
        '<div style="font-size:12px;color:var(--olive)">' + totals(m.ingredients).kcal + ' kcal</div>' +
        '<button onclick="removePM(\'' + type + '\',\'' + m.id + '\',null)" style="background:none;border:none;color:#FF6B6B;font-size:12px;cursor:pointer">✕</button>' +
        '</div></div>'
      ).join('') : '<div style="font-size:12px;color:var(--gray)">Nessun pasto.</div>') +
      '</div>';
  });
  pc.innerHTML = h;
}
...
function rPianoF() {
  ...
  h += '<div class="card-dark mb-12">' +
    ...
    '<button onclick="openFMModal(null)" class="btn btn-lime btn-sm">+ Membro</button></div>';
  if (!members.length) {
    h += '<div style="font-size:12px;color:var(--gray2)">Aggiungi i membri con nome e kcal giornaliere.</div>';
  } else {
    members.forEach(function(mb) {
      h += '<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--black3)">' +
        ...
        '<button onclick="openFMModal(\'' + mb.id + '\')" style="background:var(--black3);border:none;color:var(--gray2);padding:4px 8px;border-radius:99px;font-size:11px;cursor:pointer">✏️</button>' +
        '<button onclick="delFM(\'' + mb.id + '\')" style="background:none;border:none;color:#FF6B6B;font-size:13px;cursor:pointer">🗑️</button>' +
        '</div>';
    });
  }
  h += '</div>';
  members.forEach(function(mb) {
    h += '<div class="card-dark mb-12"><div style="font-size:15px;font-weight:800;color:var(--white);margin-bottom:12px">' + mb.name + ' <span style="font-size:11px;color:var(--gray2)">' + mb.kcal + ' kcal</span></div>';
    MEAL_TYPES.filter(t => t !== 'Snack').forEach(type => {
      ...
      if (slotMs.length) {
        slotMs.forEach(m => {
          ...
          h += '<div style="display:flex;align-items:center;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--black3)">' +
            ...
            '<div style="display:flex;align-items:center;gap:8px">' +
            '<div style="font-size:11px;color:var(--lime)">~' + Math.round(t.kcal * ratio) + ' kcal</div>' +
            '<button onclick="removePM(\'' + type + '\',\'' + m.id + '\',\'' + mb.id + '\')" style="background:none;border:none;color:#FF6B6B;font-size:11px;cursor:pointer">✕</button>' +
            '</div></div>';
        });
      } else {
...
window.setPD = i => { pianoDay = i; pianoSub === 'personale' ? rPianoP() : rPianoF(); };
window.setPW = o => { pianoWeek = o; pianoSub === 'personale' ? rPianoP() : rPianoF(); };
...
window.addToPiano = function(type, mid) {
  ...
  if (memberId) {
    ...
    closeModal('modal-piano'); rPianoF();
  } else {
    const key = pKey(pianoWeek, pianoDay);
    ...
    closeModal('modal-piano'); rPianoP();
  }
};

window.openFMModal = function(id) {
...
window.saveFM = function() {
  ...
  closeModal('modal-family-member'); rPianoF();
};
window.delFM = function(id) {
  if (!confirm('Eliminare?')) return;
...
function rSpesa() {
  ...
  const pl = plan(data), ms = meals(data), fp = fPlan(data), members = family(data);
  const mon = monOf(pianoWeek);
  const monS = mon.toLocaleDateString('it-IT', { day: 'numeric', month: 'short' });
  const sunS = new Date(mon.getTime() + 6 * 864e5).toLocaleDateString('it-IT', { day: 'numeric', month: 'short' });
  const ingMap = {};
  function addI(ingredients, factor) {
    (ingredients || []).forEach(i => { if (!ingMap[i.name]) ingMap[i.name] = 0; ingMap[i.name] += Math.round((i.grams || 0) * factor); });
  }
  for (let i = 0; i < 7; i++) {
    (pl[pKey(pianoWeek, i)] || []).forEach(e => { const m = ms.find(m => m.id === e.mealId); if (m) addI(m.ingredients, 1); });
    members.forEach(mb => {
      (fp[fpKey(pianoWeek, i, mb.id)] || []).forEach(e => {
        const m = ms.find(m => m.id === e.mealId);
        if (m) { const t = totals(m.ingredients); addI(m.ingredients, t.kcal > 0 ? mb.kcal / t.kcal : 1); }
      });
    });
  }
...
function buildModals() {
  ...
  addM(c, 'modal-meal',
    ...
    '<div class="form-group"><label class="form-label">Cerca ingrediente</label>' +
    '<input type="text" class="form-input" id="ing-search" placeholder="Digita per cercare..." oninput="onIngS(this.value)"/></div>' +
  ...
  addM(c, 'modal-family-member',
    ...
    '<div class="form-group"><label class="form-label">Nome</label><input type="text" class="form-input" id="fm-name" placeholder="Es. Sara"/></div>' +
    ...
    '<button class="btn btn-lime w-full" style="justify-content:center;margin-top:4px" onclick="saveFM()">Salva</button>' +
...
function fillCatSel(selId) {
  ...
  ingCats(D()).forEach(c => {
    const o = document.createElement('option'); o.value = c.id; o.textContent = c.emoji + ' ' + c.name;
    if (c.id === selId) o.selected = true; sel.appendChild(o);
  });
}
...
window.onIngS = function(val) {
  const q = val.trim(), res = document.getElementById('ing-res');
  if (!q) { res.innerHTML = ''; return; }
  const found = ings(D()).filter(i => i.name.toLowerCase().includes(q.toLowerCase()));
  let h = found.map(i =>
    '<button onclick="addIngToMeal(\'' + i.id + '\')" style="display:flex;align-items:center;justify-content:space-between;width:100%;background:var(--black3);border:none;border-radius:var(--radius-md);padding:10px 12px;margin-bottom:6px;cursor:pointer;text-align:left">' +
    '<span style="font-size:13px;font-weight:600;color:var(--white)">' + i.name + '</span>' +
    '<span style="font-size:11px;color:var(--gray2)">' + i.kcalPer100 + ' kcal/100g</span></button>'
  ).join('');
  if (!found.find(i => i.name.toLowerCase() === q.toLowerCase())) {
    h += '<button onclick="createAndAddIng()" style="display:flex;align-items:center;gap:8px;width:100%;background:var(--black2);border:2px dashed var(--black3);border-radius:var(--radius-md);padding:10px 12px;margin-bottom:6px;cursor:pointer">' +
...
window.addIngToMeal = function(id) {
  const i = ings(D()).find(i => i.id === id); if (!i) return;
  if (mIngreds.find(x => x.ingredientId === id)) { alert('Gia aggiunto'); return; }
...
window.updGrams = function(i, v) {
  if (!mIngreds[i]) return; mIngreds[i].grams = v; updateMTotals();
